...
function FileCard({ file, submission, onRemove, onPoll }) {
  ...
  return (
    <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-surface text-sm">
...
// ─── Result detail panel ──────────────────────────────────────────────────────
function ResultPanel({ submission, onCreateCandidate, onDownload, onDiscard, creating, downloading }) {
  if (!submission) return null
  const data = submission.parsed_data ?? {}
  const isCompleted = submission.parse_status === 'completed'
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-5 space-y-4">
      ...
      <div className="flex gap-2 pt-2 flex-wrap">
        <button
          onClick={() => onCreateCandidate(submission.id)}
          disabled={!isCompleted || creating}
          className="px-4 py-2 text-sm rounded-lg bg-brand-blue text-white hover:bg-brand-blue/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          {creating ? 'Creating…' : 'Create Candidate Profile'}
        </button>
        {hasFile && (
          <button
...
// ─── Main page ────────────────────────────────────────────────────────────────
export default function AdminCVParserPage() {
  const { user } = useAuth()
  const { submitBatch, listSubmissions, pollSubmission, createCandidate, downloadCV, getMonthlyCosts, loading } = useCVParser()
  const { show, ToastContainer } = useToast()
  const dropRef = useRef(null)

  const [files, setFiles] = useState([])       // staged files before upload
  const [submissions, setSubmissions] = useState([])  // list view
  const [fileMap, setFileMap] = useState({})   // filename → submission
  const [selected, setSelected] = useState(null)      // submission shown in result panel
  ...
  // Load cost summary (admin only)
  useEffect(() => {
    if (!isAdmin) return
    getMonthlyCosts().then(setCosts).catch(() => {})
  }, [isAdmin])
  ...
  const handleCreateCandidate = async (id) => {
    setCreating(true)
    try {
      await createCandidate(id)
      show('Candidate profile created')
      setSelected(null)
      await loadSubmissions(true)
    } catch {
      show('Failed to create candidate', 'error')
    } finally {
      setCreating(false)
    }
  }
    ...
    <AdminLayout>
      ...
      {isAdmin && costs && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <CostCard label="Total Spend" value={`$${costs.total_cost_usd?.toFixed(4)}`} />
          <CostCard label="CVs Parsed" value={costs.total_llm_calls} />
        </div>
      )}
      ...
      <div
        ...
        className={`rounded-xl border-2 border-dashed p-8 text-center transition-colors mb-6 ${
          dragging ? 'border-brand-blue bg-brand-blue/5' : 'border-border bg-surface'
        }`}
      >
