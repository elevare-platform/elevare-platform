...
def match_taxonomy(sections: DetectedSections) -> TaxonomyMatchResult:
    ...
    nigerian_certifications = [s for s in matched_skills if s.lower() in NIGERIAN_CERTS]

    # Degree matching against education section
    education_text = sections.education or ""
    matched_degrees = [
        degree for degree in DEGREE_TAXONOMY if _skill_matches(degree, education_text)
    ]
    ...
    return TaxonomyMatchResult(
        ...
        field_confidence={
            ...
            "nigerian_certifications": (
                "high" if nigerian_certifications else "not_found"
            ),
        },
    )
